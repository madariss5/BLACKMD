...
Note: Use your number in international format without any + sign, spaces, or dashes.
    ...
    async ban(sock, message, args) {
        ...
        try {
            ...
            // Parse the command for target and reason
            ...
            // Add to banned users list (for backward compatibility)
            if (!global.bannedUsers) global.bannedUsers = new Set();
            global.bannedUsers.add(targetNumber);
            
            // Add to enhanced ban data
            if (!global.bannedUsersData) global.bannedUsersData = new Map();
            global.bannedUsersData.set(targetNumber, banData);
            ...
            const formattedDate = banDate.toLocaleString();
            
            // Use the enhanced mention formatting
            const mentionData = formatPhoneForMention(targetJid);
            
            logger.info(`Banned user: ${targetNumber} (normalized from input) with reason: ${banReason}`);
            
            // Send ban notification in WhatsApp-native style with better formatting
            await safeSendMessage(sock, remoteJid, { 
                text: `*🚫 USER BANNED*\n\n*User:* ${mentionData.mention}\n*Reason:* ${banReason}\n*Banned By:* @${senderNumber}\n*Time:* ${formattedDate}`, 
                mentions: [targetJid, senderJid]
            });
            
            // Send a direct message to the banned user with an explanation if in group
            if (remoteJid.includes('@g.us')) {
                try {
                    await safeSendMessage(sock, targetJid, {
                        text: `*🚫 NOTICE: YOU HAVE BEEN BANNED*\n\n*From:* ${remoteJid.split('@')[0]}\n*Reason:* ${banReason}\n*Banned By:* @${senderNumber}\n*Time:* ${formattedDate}`,
                        mentions: [senderJid]
                    });
                } catch (dmErr) {
                    // Failed to send direct message, but continue with the ban
                    logger.debug(`Failed to send ban notification DM to ${targetNumber}: ${dmErr.message}`);
                }
            }
        } catch (err) {
