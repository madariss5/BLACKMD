...
async function getFileTypeFromBuffer(buffer) {
    try {
        // Use promise-based approach for better error handling and compatibility
        const FileType = await import('file-type').catch(err => {
            logger.error('Error importing file-type module:', err);
            return null;
        });
        
        if (FileType && typeof FileType.fileTypeFromBuffer === 'function') {
            return await FileType.fileTypeFromBuffer(buffer).catch(err => {
                logger.error('Error using fileTypeFromBuffer:', err);
                return null;
            });
        } else {
            logger.warn('file-type module imported but fileTypeFromBuffer function not found');
            return detectFileTypeFromMagicNumbers(buffer);
        }
    } catch (error) {
        logger.error('Error in getFileTypeFromBuffer:', error);
        return detectFileTypeFromMagicNumbers(buffer);
    }
}
