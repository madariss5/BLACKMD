...
class LanguageManager {
    ...
    loadLanguages() {
        try {
            ...
            // First try to load from data/translations, which is the correct location
            try {
                const deJsonPath = path.join(process.cwd(), 'data', 'translations', 'de.json');
                if (fs.existsSync(deJsonPath)) {
                    this.languages['de'] = readJsonFile(deJsonPath);
                    logger.info('Loaded German translations from data/translations/de.json');
                } else {
                    // Fallback to src/translations as a secondary location
                    try {
                        this.languages['de'] = require('../translations/de.json');
                        logger.info('Loaded German translations from src/translations/de.json');
                    } catch (fallbackErr) {
                        logger.warn('Could not load German translations from src/translations/de.json:', fallbackErr.message);
                        // Initialize with empty object if both fail
                        this.languages['de'] = {};
                    }
                }
            } catch (err) {
                logger.error('Error loading German translations:', err);
                // Initialize with empty object if both fail
                this.languages['de'] = {};
            }
            
            // Ensure all commands have translations
            for (const category in this.languages[DEFAULT_LANGUAGE]) {
                if (!this.languages['de'][category]) {
                    this.languages['de'][category] = { ...this.languages[DEFAULT_LANGUAGE][category] };
                }
                
                // Copy missing command translations from English
                for (const key in this.languages[DEFAULT_LANGUAGE][category]) {
                    if (!this.languages['de'][category][key]) {
                        this.languages['de'][category][key] = this.languages[DEFAULT_LANGUAGE][category][key];
                    }
                }
            }

            // Save translations to ensure they're complete
            this.saveTranslation(DEFAULT_LANGUAGE, this.languages[DEFAULT_LANGUAGE]);
            this.saveTranslation('de', this.languages['de']);
