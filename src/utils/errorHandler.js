 ...
 * Supports multiple languages and both group and private chats
 * 
 ...
 * @param {string} options.senderJid - The JID of the message sender (for group mentions)
 * @param {string} options.language - Override language for the error message
...
async function handleCommandError(sock, jid, error, commandName, moduleName, options = {}) {
    ...
    const {
        ...
        detailed = false,
        useRetry = true,
        senderJid = null,  // For group mentions
        language = null    // For language override
    } = options;
    
    // Get config for checking current language setting
    const config = require('../config/config');
    const isGroup = jid && jid.includes('@g.us');
    
    // Log error with JID context - this helps with debugging multi-group issues
    if (logError) {
        if (isGroup) {
            logger.error(`Error in command '${commandName}' from module '${moduleName}' in group ${jid}:`, error);
        } else {
            logger.error(`Error in command '${commandName}' from module '${moduleName}' in private chat ${jid}:`, error);
        }
    }
    ...
    try {
        // Prepare user-friendly error message based on language
        let errorMessage = '';
        const currentLang = language || config.bot.language || 'en';
        
        if (userFriendly) {
            // Get standardized error message based on category and language
...
async function safelyExecuteCommand(commandFunction, sock, message, args, commandName, moduleName) {
    ...
    try {
        ...
        // Get language either from context or global config
        const language = config.bot.language || 'en';
        
        // Prepare detailed options for error handling
        const errorOptions = {
            // Include sender JID for group mention if in a group
            senderJid: isGroup ? senderJid : null,
            // Provide current language for localized error message
            language: language,
            // Detailed errors only for owner in groups to avoid spamming
            detailed: isGroup ? isSenderBotOwner(senderJid) : true
        };
        
        await handleCommandError(sock, remoteJid, error, commandName, moduleName, errorOptions);
        return null;
    }
}
 ...
 */
function isSenderBotOwner(senderJid) {
    try {
        // Get owner number from config or environment
        const config = require('../config/config');
        const ownerNumber = process.env.OWNER_NUMBER || config.owner?.number;
