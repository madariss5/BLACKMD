...
async function messageHandler(sock, message) {
    try {
        ...
        const alternatePrefixes = config?.commands?.alternatePrefixes || [];
        const allowMultiplePrefixes = config?.commands?.allowMultiplePrefixes || false;
        
        // Fast command check - support multiple prefixes if enabled
        if (messageContent.charAt(0) === prefix || (allowMultiplePrefixes && alternatePrefixes.includes(messageContent.charAt(0)))) {
...
// Message queue system for high concurrency
const MessageQueue = require('../utils/messageQueue');
const queue = new MessageQueue(50); // Process 50 messages concurrently
    ...
    try {
        await queue.add(async () => {
            ...
            if (isGroup) {
                ...
                await handlePrivateMessage(sock, message);
            }
        });
    } catch (err) {
