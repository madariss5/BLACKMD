...
async function handleGroupMessage(sock, message) {
    try {
        ...
        // Verify bot's group membership and permissions
        try {
            const groupMetadata = await sock.groupMetadata(remoteJid);
            const botNumber = sock.user.id.split(':')[0] + '@s.whatsapp.net';
            const isBotAdmin = groupMetadata.participants.some(p => p.id === botNumber && p.admin);
            const isActive = groupMetadata.announce !== true; // Check if group is not announcement-only

            // Implement group message rate limiting
            const messageKey = `${remoteJid}_${Date.now()}`;
            const groupMessageLimit = 50; // Messages per second per group
            const currentGroupMessages = global.groupMessageCounts?.get(remoteJid) || 0;

            if (!global.groupMessageCounts) {
                global.groupMessageCounts = new Map();
            }

            if (currentGroupMessages > groupMessageLimit) {
                logger.warn(`Rate limit exceeded for group ${remoteJid}`);
                return;
            }

            global.groupMessageCounts.set(remoteJid, currentGroupMessages + 1);
            setTimeout(() => {
                global.groupMessageCounts.set(remoteJid, Math.max(0, currentGroupMessages - 1));
            }, 1000);

            if (!isActive) {
                logger.info(`Group ${remoteJid} is in announcement-only mode`);
                return;
            }

            // Add group message queuing
            if (!global.groupMessageQueues) {
                global.groupMessageQueues = new Map();
            }

            if (!global.groupMessageQueues.has(remoteJid)) {
                global.groupMessageQueues.set(remoteJid, []);
            }

            const queue = global.groupMessageQueues.get(remoteJid);
            queue.push(message);
            ...
            const processQueue = async () => {
                ...
                while (queue.length > 0) {
                    ...
                    await Promise.all(batch.map(async (nextMessage) => {
                        try {
                            if (!messageCache.has(nextMessage.key.id)) {
                                messageCache.set(nextMessage.key.id, true);
                                await processGroupMessage(sock, nextMessage);
                            }
                        } catch (err) {
            ...
            // Anti-toxic check with improved error handling
            if (settings.antitoxic && containsToxicContent(messageText)) {
                ...
                try {
                    ...
                    logger.error(`Failed to take action in group ${remoteJid}:`, actionErr);
                }
            }

        } catch (err) {
            logger.error(`Error handling group message in ${remoteJid}:`, err);
        }
    } catch (err) {
