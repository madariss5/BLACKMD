...
// Import the backupManager - needs to be imported for the backupSession function
const { backupManager } = require('../utils/backupManager');

// Constants
const DEFAULT_AUTH_FOLDER = './auth_info_baileys';
const DEFAULT_BACKUP_FOLDER = './auth_info_baileys_backup';
const BACKUP_INTERVAL = 86400000; // 24 hours (only used for emergency backups)
const MAX_BACKUPS = 3; // Only keep the most recent backup
...
class SessionManager {
    ...
    /**
     * Get credentials from the auth folder
    ...
    /**
     * Create a full backup of the current session with checksums
     * @param {string} reason The reason for the backup
     * @returns {Promise<Object>} Backup result with details
     */
    async createFullBackup(reason = 'manual') {
        try {
            // Create timestamp for unique backup directory
